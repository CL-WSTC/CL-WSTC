...
public class Evaluator_Total_Extension {
	...
	public static void main(String[] args) throws IOException {
		...
		int numofKclass=getNum(base+dataname+"_numofKclass.txt");

		Evaluator_Total_Extension e = new Evaluator_Total_Extension();
        e.script(numofKclass+1,numofKclass);
	...
	public void script(int numofclass,int numofKclass) throws IOException {
		...
		Load(GT5,base+dataname+"_5_label.txt",numofKclass);
		...
		Load(label5,"results_5/agnews_5_SeedBTM.doclabel",numofKclass); //Last period, buffer topic, Unknown Category
...
//		Load(GT5,base+dataname+"_17_label.txt",numofKclass);
...
//		Load(label5,"results_17/HuffN8_17_SeedBTM.doclabel",numofKclass);
...
//		System.out.println("5:  ");score_total(geneArray(label5), geneArray(GT5), numofclass);
	...
	public void getKnown(int[] label,int[] GT,int numofKclass) {
		for(int i=0;i<label.length;i++) {
			if(GT[i]<numofKclass) {
				known_GT.add(GT[i]);
				known_label.add(label[i]);
			}else {
				unknown_GT.add(GT[i]);
				unknown_label.add(label[i]);
			}
		}
	}
	...
	public void score_total3(int[] js,int[] js2,int numofclass) {
		...
		int count1[]=new int[numofclass+2];
		int count2[]=new int[numofclass+2];
		...
		for(int i=0;i<numofclass;i++) {
			System.out.println(i+":"+(float)count2[i]/count1[i]);
		}
	}
	
	public void Load(List<Integer> list,String name,int numofKclass) {
		File f = new File(name);
		if(!f.exists())
			return;
        ...
        try {
            ...
            for (String doc; (doc = br.readLine()) != null;) {
                if (doc.trim().length() == 0)
                    continue; 
                if(numofKclass>Integer.parseInt(doc))
                	list.add(Integer.parseInt(doc)); 
                else
                	list.add(numofKclass); 
            }
        }catch (Exception e) {
            e.printStackTrace();
        }
	...
	public void Load(List<Integer> list,String name1,String name2) {
	...
	public void Load(List<Integer> list,String name1,String name2,int numofKclass) {
		File f = new File(name1);
		if(!f.exists())
			return;
