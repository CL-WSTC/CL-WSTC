...
import approach.Seed_Update;
import approach.Seed_Update_first2period;
import basemodel.SimiBTM_withDelay_Extension;
import basemodel.WordSimilarityAvgData_Extension;

public class CLWSTC_Extension_SeedBTM {
	...
	public static void main(String[] args) throws IOException {
        ...
        CLWSTC_Extension_SeedBTM total = new CLWSTC_Extension_SeedBTM();
	...
	public void merge(String base,String dataname,String vecfile) throws IOException{
		...
		int num_buffer=2; //Buffer number
		...
		for(int time=(time_start+0);time<=(time_start+time_total);time++) {
			...
			WordSimilarityAvgData_Extension wsd = new WordSimilarityAvgData_Extension();
			wsd.script(docname,seedname,vecfile,wordsiminame,num_buffer);
			System.out.println("Similarity completed!");
			
			//2.Adaptive Threshold Learning
			int knownNumTopics=getNum(base+dataname+"_numofKclass.txt");
			int ntopics= knownNumTopics+num_buffer;
			double threshold[]=new double[knownNumTopics+num_buffer];
			if(time>time_start) {
				Threshold_Learning_Buffer up_th=new Threshold_Learning_Buffer();
				up_th.script(dataname,base,vecfile,time,num_buffer);
			}
			String thresholdname=base+dataname_t+"_threshold.txt";
			threshold=getNum_double(thresholdname,ntopics+num_buffer);
			
			//33.Classification Decision
			SimiBTM_withDelay_Extension stbtm = null;
	        int seed_limit=20;
	        try {
	            stbtm = new SimiBTM_withDelay_Extension(docname,seedname,wordsiminame,ntopics,
	            		50/ntopics+1, 0.1,200,30,dataname_t+"_"+modelname,
	            		0.0001,threshold,knownNumTopics,time,vecfile,seed_limit);
	            stbtm.inference();
	        } catch (IOException e) {
	            e.printStackTrace();
	        }
	        ...
	        LoadLabel(rejectDocFlag,folderPath+dataname_t+"_"+modelname+".rejectDocFlag");//∂¡»ÎrejectDocFlag   
	...
	public static void doc_Update(String name,int category,String word,String name_new)throws IOException {
    	...
    	for(int topic_id = 0; topic_id < 4; topic_id++) {
	...
	public double[] getNum_double(String thresholdname,int ntopics ) {
		BufferedReader br = null;
        int lineId = -1;
        double threshold[]=new double[ntopics+4];
        try {
