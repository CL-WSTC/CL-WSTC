...
//		String seeds_name=folderPath + expName + ".kbKnownSeed_prob";
		...
		//Seed Word Deletion
		//Seed word sets in the last two periods
		List<HashMap<String, Integer>> seed2performance = new ArrayList<>();
		HashMap<String, Integer> aa=new HashMap<String, Integer>();
		for(int i=0;i<ntopics;i++)
			seed2performance.add(aa);
		seed2performance=get_record(seed2performance,"results_"+(time-1)+"/SeedDelete_performance.txt","results_"+(time-2)+"/SeedDelete_performance.txt");
...
//		String seeds_pro_name=folderPath + "accept_"+time+"_"+modelname+ ".seed_prob";
		...
		String performance_delete_name=folderPath+"SeedDelete_performance.txt";//Performance record
		...
		for(int i=0;i<seedDelete_words.size();i++) {
        	...
        	//Record
        	outline+=seedDelete_id.get(i)+" "+seedDelete_words.get(i)+":"+(current-standard)+"\n";
        	delw.write(outline);delw.flush();
        	
        	if(seed2performance.get(seedDelete_id.get(i)).containsKey(seedDelete_words.get(i))) {
        		if(current>standard && seed2performance.get(seedDelete_id.get(i)).get(seedDelete_words.get(i))>=1) {
        			doc_Update(folderPath+"current_seed.txt",-1,"",folderPath+"seed_update.txt");
        			wordprobmap=wordprobmap_cur;
        			standard=current;
        			seedList_Update = new ArrayList<>();
        			LoadSeed(seedList_Update,folderPath+"current_seed.txt");
        			System.out.print(seedDelete_id.get(i)+" delete!");
        		}
        	}
		}
	...
	public List<HashMap<String, Integer>> get_record(
			List<HashMap<String, Integer>> seed2performance, String name1, String name2) {
		...
		try {
            ...
            for (String doc; (doc = br.readLine()) != null;) {
                ...
                String[] seed_str = doc.trim().split(" ");
                int topic_id=Integer.parseInt(seed_str[0]);
                String[] str = seed_str[1].trim().split(":");
                if (seed2performance.get(topic_id).containsKey(str[0])) {
                	if(Double.parseDouble(str[1])>0) 
                		seed2performance.get(topic_id).put(str[0],seed2performance.get(topic_id).get(str[0])+1);
                }else {
                	if(Double.parseDouble(str[1])>0)
                		seed2performance.get(topic_id).put(str[0], 1);
                	else
                		seed2performance.get(topic_id).put(str[0], 0);
                }
            }
