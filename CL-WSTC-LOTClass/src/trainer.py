...
class LOTClassTrainer(object):
    ...
    # set up distributed training--nccl
    ...
    def read_label_names(self, dataset_dir, label_name_file):
        ...
        for class_idx in self.label_name_dict:
            for word in self.label_name_dict[class_idx]:
                if word in self.label2class:
                    print(f"\"{word}\" used as the label name by multiple classes!")
    ...
    def category_vocabulary(self, top_pred_num=50, category_vocab_size=100, loader_name="category_vocab.pt"):
        ...
        # write category vocabulary
        cv_file = os.path.join(self.dataset_dir, 'category_vocab.txt')
        print(f"Writing category vocabulary to {cv_file}")
        f_cv = open(cv_file, 'w')
        for i, category_vocab in self.category_vocab.items():
            f_cv.write(f"{[self.inv_vocab[w] for w in category_vocab]}\n")
    ...
    def self_train_batches(self, rank, model, self_train_loader, optimizer, scheduler, test_dataset_loader):
        ...
        try:
            ...
            if rank == 0:
                ...
                # write KL divergence loss
                kl_file = os.path.join(self.dataset_dir, 'st_loss.txt')
                print(f"Writing KL divergence loss to {kl_file}")
                f_kl = open(kl_file, 'w')
                f_kl.write(str(avg_train_loss.mean().item()))
                print(f"Average training loss: {avg_train_loss.mean().item()}")
                if self.with_test_label:
            ...
            for batch in dataset_loader:
                with torch.no_grad():
                    ...
                    if return_type == "data":
                        ...
                        pred_confidence.append(torch.max(nn.Softmax(dim=-1)(logits),dim=-1)[0])
            if return_type == "data":
                all_input_ids = torch.cat(all_input_ids, dim=0)
                all_input_mask = torch.cat(all_input_mask, dim=0)
                ...
                pred_confidence = torch.cat(pred_confidence, dim=0)
    ...
    def write_results(self, loader_name="final_model.pt", out_file="out.txt"):
        ...
        # writing pred_confidence
        pred_confidence = self.inference(self.model, test_dataset_loader, 0, return_type="confidence")
        con_file = os.path.join(self.dataset_dir, "confidence.txt")
        print(f"Writing prediction confidence to {con_file}")
        f_con = open(con_file, 'w')
        for confidence in pred_confidence:
            f_con.write(str(confidence.item()) + '\n')
